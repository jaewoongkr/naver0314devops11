...
public class Ex9_DateCalender {
	...
	public static void main(String[] args) {
		...
		//Date 클래스를 이용해서 년,월,일,시분,초 얻기(JDK.1.1 이후 Deprecate 되고 Calender를 권장하나 여전히 많이 사용중
		Date date =new Date(); //디폴트로 생성시 현재 날짜와 시간을 얻는다
		int year=date.getYear()+1900;//년도는 1900 이 빠진값으로 변환
		int month=date.getMonth();//0~11반환
		int day=date.getDate();
		int week=date.getDay(); // 요일 (0: 일,1:월,2:화...6토)
		int hour=date.getHours();
		int min=date.getMinutes();
		int sec=date.getSeconds();
				
		System.out.println("Calender 를 이용하여 출력하기");
