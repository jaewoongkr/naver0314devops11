...
select ename, job from emp order by ename asc; --asc 인경우 생략가능, 오름차순 출력
...
SELECT ENAME 사원명,SAL 급여 FROM EMP;--공백없으면 "" 생략가능
...
SELECT JOB,COUNT(*),MAX(SAL),MIN(SAL),CELL(AVG(SAL)) FROM EMP GROUP BY JOB;
...
SELECT TO_CHAR(SYSDATE,'MONTH') FROM DUAL;--영어/한글로 월이 나음

--기호를 많이 쓰는데 다양한 기호로 날짜를 출력해보자
SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD') FROM DUAL;
SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD AM HH:MI') FROM DUAL;--AM 또는 PM 시간상관없이 사용
